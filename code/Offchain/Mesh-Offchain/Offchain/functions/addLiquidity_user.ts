...
//  */
export async function addLiquidityUser(utxo: UTxO) {
  const utxos = await user_wallet.getUtxos();
  const ownerUtxos = await owner_wallet.getUtxos();
  console.log("owner utxos firs", ownerUtxos[0]);
  ...
  const ownerKey = deserializeAddress(
    //@ts-ignore
    owner_wallet.addresses.baseAddressBech32?.toString(),
  );
  const userKey = deserializeAddress(
    //@ts-ignore
  ...
  const wimsAmount = 10_000n; // WIMSTOKEN que l'utilisateur envoie
  ...
  //   type: "all",
  //   scripts: [{ type: "sig", keyHash: ownerKey.pubKeyHash }],
  // };
  const nativeScript: NativeScript = {
    type: "all",
    scripts: [{ type: "sig", keyHash: ownerKey.pubKeyHash }],
  ...
  if (!user_wallet.addresses.baseAddressBech32) throw new Error("address");
  const tx = await txBuilder
    ...
    .requiredSignerHash(ownerKey.pubKeyHash)
